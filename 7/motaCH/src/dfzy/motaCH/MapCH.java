...
public class MapCH
{
	...
	private TiledCH					floorMap		= null;
	private HeroCH					hero			= null;
	private int						aheadIndex		= 0;
	private int						aheadCol		= 0;
	private int						aheadRow		= 0;

	public MapCH(HeroCH hero, Bitmap bmap)
	{
		this.hero = hero;
		floorMap = new TiledCH(TILE_NUM_COL, TILE_NUM_ROW, bmap, TILE_WIDTH, TILE_HEIGHT);
		setMap(curFloorNum);
	}
	...
	public int[] getOrgeArray()
	{
		...
		{
			repeated = false;
			if ((type = floorArray[curFloorNum][i]) >= FightCH.MIN_ORGE_INDEX)
			{
				if (type > FightCH.MAX_ORGE_INDEX)
					type -= SWITCH_OFFSET;
				if (type < MAP_ORGE1)
					continue;
				for (int j = 0; j < num; j++)
