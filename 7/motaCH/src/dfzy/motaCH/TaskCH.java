...
public class TaskCH
{
	public static final String[][] recieveDialog = {
		...
		"公主呢？我是来救公主的。",
		"公主还在里面。",
		"我答应了国王一定要把公主救出来的，现在应该怎么办呢？",
		"我把我的力量借给你，你就可以打赢那些小怪。不过，你得先去帮我找一样东西，找到了再来这里找我。",
		"找什么东西？",
		"是一个十字架，中间有一颗红色的宝石。",
		"有什么用吗？",
	...
	public TaskCH(ScreenCH gameScreen,HeroCH hero,MapCH gameMap)
	{
	...
	public void execTask(int curTask)
	{
		...
		switch (taskState[curTask])
		{
			case UNRECIEVE:
				...
				gameScreen.tu.InitText(recieveDialog[curTask][curTask2], 0, (yaCH.SCREENH-yaCH.MessageBoxH)/2, yaCH.SCREENW, yaCH.MessageBoxH, 0x0, 0xffff00, 255, yaCH.TextSize);
			...
			case RECIEVED:
				if (curTask == GET_SHENGGUANG_JIAN)
				{
					if (hero.getExperience() >= 500)
					{
						mbtask = true;	
						gameScreen.mshowDialog = true;
						gameScreen.tu.InitText(finishedDialog[curTask][curTask2], 0, (yaCH.SCREENH-yaCH.MessageBoxH)/2, yaCH.SCREENW, yaCH.MessageBoxH, 0x0, 0xffff00, 255, yaCH.TextSize);
	...
	private void recieveTask()
	{
		...
		switch (curTask)
		{
			case FIND_CROSS:
				gameMap.remove();
				gameMap.changeCell(92, MapCH.MAP_ANGLE);
				hero.addYellowKey();
				hero.addBlueKey();
				hero.addRedKey();
				break;
			...
			case RESCUE_PRINCESS:
				gameMap.changeCell(120, MapCH.MAP_UPSTAIR);
				break;
			case FIGHT_BOSS:
				gameMap.remove();
				taskState[curTask]++;
	...
	private void finishTask()
	{
		...
		switch (curTask)
		{
			...
			case FIND_AX:
				gameMap.remove();
				gameMap.remove(18, 49, MapCH.MAP_PRINCESS);
				gameMap.remove(18, 60, MapCH.MAP_BARRIER);
				gameMap.remove(18, 71, MapCH.MAP_RED_DOOR);
				gameMap.remove(18, 82, MapCH.MAP_RED_DOOR);
				break;
			case GET_QINGFEND_JIAN:
				gameScreen.showMessage(hero.takeGem(MapCH.MAP_SWORD3));
				break;
			case GET_HUANGJIN_DUN:
				gameScreen.showMessage(hero.takeGem(MapCH.MAP_SHIELD3));
				break;
			case GET_SHENGGUANG_JIAN:
				gameScreen.showMessage(hero.takeGem(MapCH.MAP_SWORD4));
				break;
			case GET_XINGGUANG_DUN:
				gameScreen.showMessage(hero.takeGem(MapCH.MAP_SHIELD4));
				break;
			case RESCUE_PRINCESS:
				gameScreen.end();
				break;
			case FIGHT_BOSS:
				if (gameScreen.fight(MapCH.MAP_ORGE31) == true)
				{
					gameMap.changeCell(81, MapCH.MAP_ROAD);
					gameMap.changeCell(83, MapCH.MAP_ROAD);
